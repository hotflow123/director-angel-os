...
import {
  ...
  type PromptSection,
  type PromptSectionContributor,
  PromptSectionRegistry,
} from "@hotflow/context";
import {
  type BuildDirectorContextualRecallInput,
  type DirectorCapabilityContextPacket,
...
export function createDefaultRuntimePromptSections(
  config: Pick<
    HotflowConfig,
    | "defaultModel"
    | "defaultProvider"
...
export function createDefaultRuntimePromptRegistry(
  config: Pick<
    HotflowConfig,
    | "defaultModel"
    | "defaultProvider"
...
export function createDefaultRuntimeContextAssembler(
  config: Pick<
    HotflowConfig,
    | "defaultModel"
    | "defaultProvider"
...
export interface RuntimePromptSectionContributorSources {
  contextualRecall?: {
    build(
      input: Pick<BuildDirectorContextualRecallInput, "skillSections" | "userText">,
    ):
      | DirectorCapabilityContextPacket
      | Omit<BuildDirectorContextualRecallInput, "skillSections" | "userText">
      | undefined;
  };
  skillPromptIndex?: {
    buildSections(input: unknown): readonly PromptSection[];
  };
}

export function createDefaultRuntimePromptSectionContributors(
  sources: RuntimePromptSectionContributorSources,
): readonly PromptSectionContributor[] {
...
function isDirectorCapabilityContextPacket(
  value: unknown,
): value is DirectorCapabilityContextPacket {
  return (
    typeof value === "object" &&
    value !== null &&
...
function renderOutputStyleGuidance(style: HotflowConfig["outputStyle"]): string {
  switch (style) {
    case "concise":
...
function renderPermissionModeGuidance(mode: HotflowConfig["permissionMode"]): string {
  switch (mode) {
    case "allow":
...
function renderResponseLanguageGuidance(language: string): string {
