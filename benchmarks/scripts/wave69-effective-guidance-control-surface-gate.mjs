...
function buildWorkspace(target, env) {
  ...
  if (result.status !== 0) {
    fail(
      `${target} build failed for wave69 effective guidance control surface gate.`,
      result.stderr,
    );
  }
}
...
async function runGate() {
  ...
  const workspaceRoot = mkdtempSync(join(tmpdir(), "hotflow-wave69-effective-guidance-control-"));
  const dataDir = join(workspaceRoot, ".hotflow");
  const sessionDbPath = join(dataDir, "sessions", "wave69-effective-guidance-control.sqlite");
  const sessionId = "sess_wave69_effective_guidance_control_surface";

  mkdirSync(join(dataDir, "sessions"), { recursive: true });
  writeFileSync(
    join(workspaceRoot, "README.md"),
    "Wave 69 effective guidance control surface.\n",
    "utf8",
  );
  ...
  try {
    ...
    if (failures.length > 0) {
      fail("wave69 effective guidance control surface gate failed.", failures.join("\n"));
    }

    mkdirSync(resultsDir, { recursive: true });
    writeFileSync(
      join(resultsDir, "wave69-effective-guidance-control-surface-gate.json"),
      JSON.stringify(
        {
          gate: "wave69-effective-guidance-control-surface-gate",
          sessionId,
          summary,
  ...
  process.stdout.write("wave69 effective guidance control surface gate passed.\n");
}
