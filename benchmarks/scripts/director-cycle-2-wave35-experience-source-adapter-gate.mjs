...
const suiteId = "director-cycle-2-wave35-experience-source-adapter-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle2_wave35_contracts_build",
    label: "contracts build publishes review-gated experience source contracts",
    args: ["--filter", "@hotflow/contracts", "build"],
  },
  {
    id: "cycle2_wave35_contracts_tests",
    label: "contracts tests cover experience candidates, review, promotion, and rollback records",
    args: ["--filter", "@hotflow/contracts", "test", "--", "--runInBand", "test/learning.test.ts"],
  },
  {
    id: "cycle2_wave35_director_knowledge_build",
    label: "director-knowledge build compiles local reference repository experience adapter",
    args: ["--filter", "@hotflow/director-knowledge", "build"],
  },
  {
    id: "cycle2_wave35_director_knowledge_tests",
    label: "director-knowledge tests materialize review-gated local reference repo candidates",
    args: [
      "--filter",
      ...
      "--",
      "--runInBand",
      "test/experience-source.test.ts",
    ],
  },
];
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 2 Wave 35 experience source adapter gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 2 Wave 35 experience source adapter gate failed.",
