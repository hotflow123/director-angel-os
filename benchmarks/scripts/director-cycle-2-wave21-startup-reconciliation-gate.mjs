...
const suiteId = "director-cycle-2-wave21-startup-reconciliation-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle2_wave21_contracts_build",
    label: "Contracts build exposes startup reconciliation reason metadata",
    args: ["--filter", "@hotflow/contracts", "build"],
  },
  {
    id: "cycle2_wave21_tasks_core_build",
    label: "tasks-core build compiles startup reconciliation primitives",
    args: ["--filter", "@hotflow/tasks-core", "build"],
  },
  {
    id: "cycle2_wave21_startup_reconciliation_tests",
    label: "tasks-core startup reconciliation covers board and session-backed recovery",
    args: ["--filter", "@hotflow/tasks-core", "test", "--", "src/startup-reconciliation.test.ts"],
  },
];
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 2 Wave 21 startup reconciliation gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 2 Wave 21 startup reconciliation gate failed.",
