...
const suiteId = "director-cycle-2-wave39-web-search-learning-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle2_wave39_contracts_build",
    label: "contracts build publishes web-search experience source kind",
    args: ["--filter", "@hotflow/contracts", "build"],
  },
  {
    id: "cycle2_wave39_contracts_learning_tests",
    label: "contracts tests cover web-search source declarations",
    args: ["--filter", "@hotflow/contracts", "test", "--", "--runInBand", "test/learning.test.ts"],
  },
  {
    id: "cycle2_wave39_director_knowledge_build",
    label: "director-knowledge build compiles web-search self-learning adapter",
    args: ["--filter", "@hotflow/director-knowledge", "build"],
  },
  {
    id: "cycle2_wave39_director_knowledge_tests",
    label: "director-knowledge tests search by topic and store fetched result pages",
    args: [
      "--filter",
  ...
  {
    id: "cycle2_wave39_cli_build",
    label: "cli build exposes knowledge learn --query",
    args: ["--filter", "@hotflow/cli", "build"],
  },
  {
    id: "cycle2_wave39_cli_tests",
    label: "cli tests cover injected web search learning provider",
    args: [
      "--filter",
      "@hotflow/cli",
      "test",
  ...
  {
    id: "cycle2_wave39_targeted_format",
    label: "targeted Biome check covers web-search learning files",
    args: [
      "exec",
      "biome",
      "check",
      ...
      "benchmarks/scripts/director-cycle-2-wave39-web-search-learning-gate.mjs",
    ],
  },
];
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 2 Wave 39 web search learning gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 2 Wave 39 web search learning gate failed.",
