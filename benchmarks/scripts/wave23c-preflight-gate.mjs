...
const PROBE_CODE = `
import { runCliPreflightViaControlPlane } from "../src/preflight.ts";
      ...
      async dispatch(action) {
        ...
        return {
          ...
          data: {
            ...
            readiness: "ready",
            sessionId: "operator_preflight",
            summaryText: "probe preflight pass",
            commands: [],
            environment: {
              profile: "development",
            ...
            directorExecution: {
              status: "pass",
              summaryText: "No persisted director execution report exists yet.",
              suggestedCommands: [],
              routeCounts: {
                healthy: 0,
                degraded: 0,
            ...
            surfaces: {
              environment: {
                status: "pass",
                summaryText: "workspace paths are visible",
              },
              doctor: {
                status: "pass",
                summaryText: "0 checks (0 pass, 0 warn, 0 fail).",
                counts: {
                  pass: 0,
                  warn: 0,
                  fail: 0,
...
function runProbe(cliDir) {
  ...
  const probePath = join(probeDir, `wave23c-preflight-probe-${process.pid}.ts`);
...
function seedDirectorExecutionArtifacts(workspaceRoot, preset) {
  if (preset !== "reroutable_warn") {
    return;
  }
...
function runPreflightCase(benchmarkCase, repoRoot, tempRoot) {
  ...
  const result = runHotflowCli(repoRoot, benchmarkCase.command ?? ["preflight", "--json"], env);
  ...
  if (!payload) {
    failures.push("Expected JSON preflight report but stdout was not valid JSON.");
  }
  ...
  return {
    ...
    command: benchmarkCase.command ?? ["preflight", "--json"],
...
function runCase(benchmarkCase, paths) {
  if (benchmarkCase.mode === "preflight") {
    const tempRoot = mkdtempSync(join(tmpdir(), "hotflow-wave23c-preflight-"));
    try {
      return runPreflightCase(benchmarkCase, paths.repoRoot, tempRoot);
    } finally {
  ...
  return {
    ...
    ok: false,
    durationMs: 0,
    failures: [`Unknown Wave 23c mode: ${String(benchmarkCase.mode)}`],
  };
}

const paths = getBenchmarksPaths();
const fixturePath = resolve(paths.benchmarksDir, "fixtures", "wave23c-preflight-gate.json");
...
const report = {
  suiteId: typeof fixture.suiteId === "string" ? fixture.suiteId : "wave23c-preflight-gate",
...
const outputPath = resolve(paths.resultsDir, "wave23c-preflight-gate-latest.json");
writeFileSync(outputPath, JSON.stringify(report, null, 2));

process.stdout.write(
  `Wave 23c preflight gate completed: ${summary.totalRuns} runs, failed ${summary.failedRuns}\n`,
);
