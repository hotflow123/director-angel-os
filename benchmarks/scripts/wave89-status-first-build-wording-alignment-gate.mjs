...
function buildWorkspace(target, env) {
  ...
  if (result.status !== 0) {
    fail(
      `${target} build failed for wave89 status first-build wording alignment gate.`,
      result.stderr,
    );
  }
}
...
function seedSession(sessionDbPath, sessionId, workspaceRoot) {
  ...
  try {
    const turnId = "turn_wave89_status_first_build_wording";
    store.createSession({
      sessionId,
      metadata: {
        runtime: {
    ...
    store.appendJournal(sessionId, {
      ...
      payload: {
        items: [{ id: "todo_wave89", content: "status first build wording", status: "todo" }],
      },
      createdAtMs: 100,
    });
    ...
    writeFileSync(
      ...
      "Wave 89 status first-build wording.\n",
      "utf8",
    );
  } finally {
    store.close();
...
async function runGate() {
  ...
  const workspaceRoot = mkdtempSync(join(tmpdir(), "hotflow-wave89-status-first-build-"));
  const dataDir = join(workspaceRoot, ".hotflow");
  const sessionDbPath = join(
    dataDir,
    "sessions",
    "wave89-status-first-build-wording-alignment.sqlite",
  );
  const sessionId = "sess_wave89_status_first_build_wording";
  ...
  try {
    ...
    writeFileSync(
      join(resultsDir, "wave89-status-first-build-wording-alignment-gate.json"),
      JSON.stringify(
        {
          gate: "wave89-status-first-build-wording-alignment-gate",
          workspaceRoot,
          repoRoot,
          statusRun,
          auditSummary,
    ...
    if (failures.length > 0) {
      fail("wave89 status first-build wording alignment gate failed.", failures.join("\n"));
    }

    process.stdout.write("wave89 status first-build wording alignment gate passed.\n");
  } finally {
