...
const suiteId = "director-phase-p2-wave14-run-state-surface-alignment-gate";
...
const cases = [
  ...
  },
  {
    ...
    label:
      "Main CLI aligns run report and run explain with created and paused holding-state taxonomy",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave14_acceptance_surface",
    label: "Director acceptance requires the run-state surface alignment artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 14 run-state surface alignment gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 14 run-state surface alignment gate failed.",
