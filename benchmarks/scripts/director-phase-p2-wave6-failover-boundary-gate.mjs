...
const cases = [
  ...
  },
  {
    ...
    label: "Worker host-api path reroutes to an approved alternate adapter",
  ...
  },
  {
    ...
    label: "CLI surfaces expose reroute guidance without automatic failover",
    args: [
      "--filter",
      "@hotflow/cli",
      "test",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 6 failover boundary gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 6 failover boundary gate failed.",
