...
const suiteId = "director-cycle-2-wave36-experience-review-store-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle2_wave36_contracts_build",
    label: "contracts build publishes experience review and promotion records",
    args: ["--filter", "@hotflow/contracts", "build"],
  },
  {
    id: "cycle2_wave36_director_knowledge_build",
    label: "director-knowledge build compiles file-backed experience review store",
    args: ["--filter", "@hotflow/director-knowledge", "build"],
  },
  {
    id: "cycle2_wave36_director_knowledge_tests",
    label:
      "director-knowledge tests gate promotion on accepted review and rollback known promotions",
    args: [
      "--filter",
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 2 Wave 36 experience review store gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 2 Wave 36 experience review store gate failed.",
