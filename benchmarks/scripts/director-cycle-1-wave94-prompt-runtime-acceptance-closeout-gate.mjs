...
const suiteId = "director-cycle-1-wave94-prompt-runtime-acceptance-closeout-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle1_wave94_director_acceptance_surface",
    label: "Director acceptance requires Cycle 1 Wave 83-93 prompt/runtime closeout artifacts",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/director-acceptance.test.ts"],
  },
  {
    id: "cycle1_wave94_shell_acceptance_surface",
    label: "CLI acceptance output surfaces Cycle 1 Wave 83-93 prompt/runtime closeout checks",
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 1 Wave 94 prompt/runtime acceptance closeout gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 1 Wave 94 prompt/runtime acceptance closeout gate failed.",
