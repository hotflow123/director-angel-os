...
function seedPromptSectionLabelSession(sessionDbPath, sessionId) {
  ...
  try {
    store.createSession({
      ...
      metadata: {
        runtime: {
          latestTurn: {
            turnId: "turn_prompt_section_label_gate",
    ...
    store.appendJournal(sessionId, {
      ...
      payload: {
        items: [
          { id: "todo_prompt_labels", content: "prompt section labels todo", status: "todo" },
        ],
      },
      createdAtMs: 100,
    });
...
async function runGate() {
  ...
  const workspaceRoot = mkdtempSync(join(tmpdir(), "hotflow-wave36-prompt-section-label-summary-"));
  const dataDir = join(workspaceRoot, ".hotflow");
  const sessionDbPath = join(dataDir, "sessions", "wave36-prompt-section-label-summary.sqlite");
  const sessionId = "sess_wave36_prompt_section_label_summary";

  mkdirSync(join(dataDir, "sessions"), { recursive: true });
  writeFileSync(
    join(workspaceRoot, "README.md"),
    "Wave 36 prompt section label summary benchmark.\n",
    "utf8",
  );
  ...
  if (sessionsBuild.status !== 0) {
    fail(
      "sessions build failed for wave36 prompt section label summary gate.",
  ...
  if (controlPlaneBuild.status !== 0) {
    fail(
      "control-plane build failed for wave36 prompt section label summary gate.",
  ...
  if (cliBuild.status !== 0) {
    fail("cli build failed for wave36 prompt section label summary gate.", cliBuild.stderr);
  }

  try {
    seedPromptSectionLabelSession(sessionDbPath, sessionId);

    const explainRun = runHotflowCliCommand(
      ["control", "prompt-explain", sessionId, "--turn", "turn_prompt_section_label_gate"],
      env,
    );
    ...
    const failures = [
      ...
      ...verifyRun(statusRun, [
        ...
        "Prompt focus: tool results, session guidance, tool runtime guidance",
        "Prompt omissions: runtime degradations",
        "Prompt changes: +tool results +tool runtime guidance -user input",
      ]),
    ];
    ...
    writeFileSync(
      join(resultsDir, "wave36-prompt-section-label-summary-gate-latest.json"),
      JSON.stringify(
        {
          gate: "wave36-prompt-section-label-summary",
          workspaceRoot,
          repoRoot,
          runs: {
            explain: explainRun,
    ...
    if (failures.length > 0) {
      fail("wave36 prompt section label summary gate failed.", failures.join("\n"));
    }
  } finally {
    rmSync(workspaceRoot, { recursive: true, force: true });
  }

  process.stdout.write("wave36 prompt section label summary gate passed.\n");
}
