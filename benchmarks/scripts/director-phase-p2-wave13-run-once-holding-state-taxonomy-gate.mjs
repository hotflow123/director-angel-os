...
const suiteId = "director-phase-p2-wave13-run-once-holding-state-taxonomy-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "phase_p2_wave13_worker_run_once_contract",
    label: "Director worker still exposes the bounded run-once contract for local execution",
    args: ["--filter", "@hotflow/director-worker", "test", "--", "tests/run-once.test.ts"],
  },
  {
    id: "phase_p2_wave13_cli_run_once_holding_state_surface",
    label: "Main CLI classifies created and paused holding states after a bounded run-once pass",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave13_acceptance_surface",
    label: "Director acceptance requires the run-once holding-state taxonomy artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 13 run-once holding-state taxonomy gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 13 run-once holding-state taxonomy gate failed.",
