...
const suiteId = "director-cycle-2-wave30-resume-reasoning-recovery-symmetry-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "cycle2_wave30_cli_build",
    label:
      "CLI build compiles resume reasoning recovery selection into observation and shell surfaces",
    args: ["--filter", "@hotflow/cli", "build"],
  },
  {
    id: "cycle2_wave30_cli_tests",
    label:
      "CLI tests cover stream-first reasoning recovery, latest-turn fallback, control-plane resume snapshots, and resume audit closure",
    args: [
      "--filter",
      "@hotflow/cli",
      "test",
...
async function main() {
  ...
  process.stdout.write(
    `Director Cycle 2 Wave 30 resume reasoning recovery symmetry gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Cycle 2 Wave 30 resume reasoning recovery symmetry gate failed.",
