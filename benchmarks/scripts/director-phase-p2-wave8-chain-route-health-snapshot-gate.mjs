...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 8 chain route health snapshot gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 8 chain route health snapshot gate failed.",
