...
const fixturePath = resolve(benchmarksDir, "fixtures", "wave1-quality-gate.json");
...
if (buildResult.status !== 0) {
  fail(
    "Failed to build @hotflow/testkit before quality gate.",
    `${buildResult.stderr}\n${buildResult.stdout}`,
  );
}
...
const createWave1QualityBenchmarkCases = testkitModule.createWave1QualityBenchmarkCases;
const runBenchmarkGate = testkitModule.runBenchmarkGate;

if (
  typeof createWave1QualityBenchmarkCases !== "function" ||
  typeof runBenchmarkGate !== "function"
) {
  fail("Missing benchmark exports from @hotflow/testkit.");
}

const definedCases = createWave1QualityBenchmarkCases();
const selectedCases =
  caseIds.length > 0
...
if (selectedCases.length === 0) {
  fail("No Wave 1 quality cases selected from fixture.");
}

const iterations = toPositiveInteger(
  process.env.HF_WAVE1_GATE_ITERATIONS,
  toPositiveInteger(fixture.iterations, 1),
);
const report = await runBenchmarkGate({
  suiteId: typeof fixture.suiteId === "string" ? fixture.suiteId : "wave1-quality-gate",
  cases: selectedCases,
  iterations,
});

mkdirSync(resultsDir, { recursive: true });
const outputPath = resolve(resultsDir, "wave1-quality-gate-latest.json");
writeFileSync(outputPath, JSON.stringify(report, null, 2));

process.stdout.write(
  `Wave 1 quality gate completed: ${report.summary.totalRuns} runs, failed ${report.summary.failedRuns}\n`,
);
