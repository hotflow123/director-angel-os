...
const suiteId = "director-phase-p2-wave10-worker-once-operator-entry-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "phase_p2_wave10_director_worker_local_run_once",
    label: "Director worker run-once keeps the local execution lane green",
    args: ["--filter", "@hotflow/director-worker", "test", "--", "tests/run-once.test.ts"],
  },
  {
    id: "phase_p2_wave10_cli_director_run_once_surface",
    label: "Main CLI exposes director run once and executes a local run through the worker lane",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave10_acceptance_surface",
    label: "Acceptance surface requires the worker once operator entry artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 10 worker once operator entry gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 10 worker once operator entry gate failed.",
