...
const suiteId = "director-phase-p2-wave9-chain-route-doctor-visibility-gate";
...
const cases = [
  {
    ...
    label:
      "Director doctor classifies healthy, degraded, reroutable, exhausted, and blocked routes",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/director-doctor.test.ts"],
  },
  {
    id: "phase_p2_wave9_cli_director_doctor_surface",
    label: "CLI director doctor command renders chain route health visibility",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave9_acceptance_surface",
    label: "Acceptance surface requires the chain route doctor visibility artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 9 chain route doctor visibility gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 9 chain route doctor visibility gate failed.",
