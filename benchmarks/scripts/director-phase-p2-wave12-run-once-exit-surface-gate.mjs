...
const suiteId = "director-phase-p2-wave12-run-once-exit-surface-gate";
...
const cases = [
  ...
  },
  {
    ...
    label:
      "Main CLI renders operator-readable run-once exit status, next action, and follow-up commands",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave12_acceptance_surface",
    label: "Director acceptance requires the run-once exit-surface artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 12 run-once exit surface gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 12 run-once exit surface gate failed.",
