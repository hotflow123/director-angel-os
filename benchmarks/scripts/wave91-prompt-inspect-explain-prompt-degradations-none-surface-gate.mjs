...
function buildWorkspace(target, env) {
  ...
  if (result.status !== 0) {
    fail(
      `${target} build failed for wave91 prompt-inspect/prompt-explain prompt degradations none surface gate.`,
      result.stderr,
    );
  }
}
...
function seedSession(sessionDbPath, sessionId, workspaceRoot) {
  ...
  try {
    const turnId = "turn_wave91_prompt_degradations_none";
    store.createSession({
      sessionId,
      metadata: {
        runtime: {
    ...
    writeFileSync(
      ...
      "Wave 91 prompt-inspect/prompt-explain prompt degradations none surface.\n",
      "utf8",
    );
  } finally {
    store.close();
...
async function runGate() {
  ...
  const workspaceRoot = mkdtempSync(join(tmpdir(), "hotflow-wave91-prompt-degradations-none-"));
  const dataDir = join(workspaceRoot, ".hotflow");
  const sessionDbPath = join(
    dataDir,
    "sessions",
    "wave91-prompt-inspect-explain-prompt-degradations-none-surface.sqlite",
  );
  const sessionId = "sess_wave91_prompt_degradations_none";
  ...
  try {
    ...
    const promptInspectRun = runHotflowCliCommand(
      [
        ...
        sessionId,
        "--turn",
        "turn_wave91_prompt_degradations_none",
        "--step",
        "0",
      ],
      env,
    ...
    writeFileSync(
      join(resultsDir, "wave91-prompt-inspect-explain-prompt-degradations-none-surface-gate.json"),
      JSON.stringify(
        {
          gate: "wave91-prompt-inspect-explain-prompt-degradations-none-surface-gate",
          workspaceRoot,
          repoRoot,
          runs: {
    ...
    if (failures.length > 0) {
      fail(
        "wave91 prompt-inspect/prompt-explain prompt degradations none surface gate failed.",
        failures.join("\n"),
      );
    }

    process.stdout.write(
      "wave91 prompt-inspect/prompt-explain prompt degradations none surface gate passed.\n",
    );
  } finally {
