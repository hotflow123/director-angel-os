...
const suiteId = "director-phase-p2-wave7-route-recovery-audit-closure-gate";
const latestPath = resolve(resultsDir, `${suiteId}-latest.json`);

const cases = [
  {
    id: "phase_p2_wave7_execution_service_route_recovery_events",
    label: "Execution service emits structured route recovery event payloads",
    args: [
      "--filter",
  ...
  },
  {
    ...
    label: "CLI audit renders structured route recovery evidence",
    args: ["--filter", "@hotflow/cli", "test", "--", "src/shell.test.ts"],
  },
  {
    id: "phase_p2_wave7_acceptance_surface",
    label: "Acceptance surface requires the route recovery audit closure artifact",
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 7 route recovery audit closure gate completed: ${caseResults.length} cases, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 7 route recovery audit closure gate failed.",
