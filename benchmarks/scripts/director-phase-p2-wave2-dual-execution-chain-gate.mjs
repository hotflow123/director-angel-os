...
const fixturePath = resolve(
  benchmarksDir,
  "fixtures",
  "director-phase-p2-wave2-dual-execution-chain-gate.json",
);
...
function runBuild(target) {
  ...
  if (result.status !== 0) {
    fail(
      `Failed to build ${target} before Director Phase P2 Wave 2 gate.`,
...
function buildExecutionIntake(snapshot) {
  const objective =
    ...
    `Phase P2 Wave 2 execution request for ${snapshot.group.groupId}`;
  ...
  return {
    ...
    deliverables: ["Submit two bounded execution adapter requests."],
    notes: ["Benchmark synthesized intake payload for Director Angel Phase P2 Wave 2."],
  };
}
...
function normalizeBlueprintForExecutionRun(blueprint, adapterId, runOverrides = {}) {
  ...
  const shotPlanner =
    blueprint?.actionGraph?.nodes?.find((node) => node.role === "shot-planner") ?? null;
...
async function requestJson(url, options = {}) {
  let response;
  let lastError = null;
  const maxAttempts = options.method === undefined || options.method === "GET" ? 3 : 1;
  ...
  if (response === undefined) {
    throw lastError ?? new Error(`Request failed before response: ${url}`);
  }
...
function resolveBridgeResponseRequestId(behavior, requestIndex) {
  if (Array.isArray(behavior.requestIds)) {
    return behavior.requestIds[requestIndex];
  }
...
async function startBridgeServer(behavior, expectedSecret) {
  ...
  const server = createHttpServer(async (request, response) => {
    ...
    const requestId = resolveBridgeResponseRequestId(behavior, requestIndex);
...
async function runCase(benchmarkCase) {
  ...
  const workspaceRoot = mkdtempSync(join(tmpdir(), `director-phase-p2-wave2-${benchmarkCase.id}-`));
  const dataDir = join(workspaceRoot, ".hotflow");
  const sessionDbPath = join(dataDir, "sessions", `${benchmarkCase.id}.sqlite`);
  const commandRuns = [];
  const failures = [];
  const bridgeSecret = "bench-p2-wave2-secret";
  ...
  try {
    ...
    if (!shotPlanner) {
      failures.push("reviewed blueprint did not include a shot-planner node.");
    }
    for (const [role, node] of [
      ["script-planner", scriptPlanner],
      ["shot-planner", shotPlanner],
    ]) {
      if (!node) {
        continue;
      }
      if (node.selectedAdapter !== benchmarkCase.expected.adapterId) {
        failures.push(
          `${role} selectedAdapter expected ${benchmarkCase.expected.adapterId} but received ${node.selectedAdapter ?? "(missing)"}.`,
        );
      }
      if (!node.allowedAdapters?.includes(benchmarkCase.expected.adapterId)) {
        failures.push(
          `${role} allowedAdapters should include ${benchmarkCase.expected.adapterId}.`,
        );
      }
      if (node.actionClass !== "generate") {
        failures.push(
          `${role} actionClass expected generate but received ${node.actionClass ?? "(missing)"}.`,
        );
      }
    }
    ...
    const createdScriptPlanner = findAssignmentByRole(createdRun?.assignments, "script-planner");
    const createdShotPlanner = findAssignmentByRole(createdRun?.assignments, "shot-planner");
    ...
    if (createdShotPlanner?.selectedAdapter !== benchmarkCase.expected.adapterId) {
      failures.push(
        `created run shot-planner selectedAdapter expected ${benchmarkCase.expected.adapterId} but received ${createdShotPlanner?.selectedAdapter ?? "(missing)"}.`,
      );
    }
    if (
      createdScriptPlanner?.assignmentId !== undefined &&
      createdShotPlanner?.dependsOn?.includes(createdScriptPlanner.assignmentId) !== true
    ) {
      failures.push("created run shot-planner should depend on the script-planner assignment.");
    }
    ...
    if (benchmarkCase.expected.bridgeRequestCount > 0) {
      const expectedBridgeRoles = benchmarkCase.expected.bridgeRoles ?? [];
      const assignmentIdByRole = new Map(
        [
          ["script-planner", scriptPlanner?.assignmentId],
          ["shot-planner", shotPlanner?.assignmentId],
        ].filter((entry) => entry[1] !== undefined),
      );
      const observedAssignmentIds = [];

      for (const [index, bridgeRequest] of bridgeServer.requests.entries()) {
        if (!bridgeRequest?.authorizationMatched) {
          failures.push(
            `bridge request ${index + 1} did not receive the expected bearer credential.`,
          );
        }
        if (bridgeRequest?.body?.schemaId !== "director.execution.http-json-request.v1") {
          failures.push(
            `bridge request ${index + 1} schemaId expected director.execution.http-json-request.v1 but received ${bridgeRequest?.body?.schemaId ?? "(missing)"}.`,
          );
        }
        if (bridgeRequest?.body?.adapterId !== benchmarkCase.expected.adapterId) {
          failures.push(
            `bridge request ${index + 1} adapterId expected ${benchmarkCase.expected.adapterId} but received ${bridgeRequest?.body?.adapterId ?? "(missing)"}.`,
          );
        }
        const expectedRole = expectedBridgeRoles[index];
        if (expectedRole !== undefined && bridgeRequest?.body?.role !== expectedRole) {
          failures.push(
            `bridge request ${index + 1} role expected ${expectedRole} but received ${bridgeRequest?.body?.role ?? "(missing)"}.`,
          );
        }
        if (bridgeRequest?.body?.actionClass !== "generate") {
          failures.push(
            `bridge request ${index + 1} actionClass expected generate but received ${bridgeRequest?.body?.actionClass ?? "(missing)"}.`,
          );
        }
        if (
          typeof bridgeRequest?.body?.workerId !== "string" ||
          bridgeRequest.body.workerId.length === 0
        ) {
          failures.push(`bridge request ${index + 1} should include a workerId.`);
        }
        const expectedAssignmentId =
          expectedRole === undefined ? undefined : assignmentIdByRole.get(expectedRole);
        if (
          expectedAssignmentId !== undefined &&
          bridgeRequest?.body?.assignmentId !== expectedAssignmentId
        ) {
          failures.push(
            `bridge request ${index + 1} assignmentId expected ${expectedAssignmentId} but received ${bridgeRequest?.body?.assignmentId ?? "(missing)"}.`,
          );
        }
        if (bridgeRequest?.body?.assignmentId !== undefined) {
          observedAssignmentIds.push(bridgeRequest.body.assignmentId);
        }
      }

      if (new Set(observedAssignmentIds).size !== observedAssignmentIds.length) {
        failures.push("dual execution chain bridge requests should map to distinct assignmentIds.");
      }
    }
    ...
    if (!runResult.response.ok) {
      ...
      const finalAssignments = new Map(
        (runResult.body?.assignments ?? []).map((assignment) => [assignment.role, assignment]),
      );
      if (benchmarkCase.expected.bridgeRequestCount > 0) {
        for (const [index, role] of (benchmarkCase.expected.bridgeRoles ?? []).entries()) {
          const finalAssignment = finalAssignments.get(role) ?? null;
          const expectedRequestId = resolveBridgeResponseRequestId(
            benchmarkCase.bridgeBehavior,
            index,
          );
          if (!finalAssignment?.result?.bridgeExecution) {
            failures.push(`final run should include bridgeExecution on the ${role} assignment.`);
            continue;
          }
          if (
            finalAssignment.result.bridgeExecution.response?.statusCode !==
            benchmarkCase.bridgeBehavior.statusCode
          ) {
            failures.push(
              `${role} bridgeExecution.response.statusCode expected ${benchmarkCase.bridgeBehavior.statusCode} but received ${finalAssignment.result.bridgeExecution.response?.statusCode ?? "(missing)"}.`,
            );
          }
          if (
            expectedRequestId !== undefined &&
            finalAssignment.result.bridgeExecution.response?.requestId !== expectedRequestId
          ) {
            failures.push(
              `${role} bridgeExecution.response.requestId expected ${expectedRequestId} but received ${finalAssignment.result.bridgeExecution.response?.requestId ?? "(missing)"}.`,
            );
          }
        }
      } else {
        for (const role of benchmarkCase.expected.bridgeRoles ?? []) {
          if (finalAssignments.get(role)?.result?.bridgeExecution !== undefined) {
            failures.push(`preview fallback run should not persist bridgeExecution on ${role}.`);
          }
        }
      }
    }
        ...
        ? {
            ...
            shotPlanner:
              shotPlanner === null
                ? null
                : {
                    assignmentId: shotPlanner.assignmentId,
                    objective: shotPlanner.objective,
                    deliverable: shotPlanner.deliverable,
                    selectedAdapter: shotPlanner.selectedAdapter,
                  },
          }
        : null,
      bridgeRequests: bridgeServer.requests.map((request) => ({
        authorizationMatched: request.authorizationMatched,
        body:
          request.body === null
            ? null
            : {
                ...
                actionClass: request.body.actionClass,
...
async function main() {
  ...
  process.stdout.write(
    `Director Phase P2 Wave 2 dual execution chain gate completed: ${caseResults.length} runs, failed ${failed.length}\n`,
  );
  process.stdout.write(`Report: ${latestPath}\n`);

  if (failed.length > 0) {
    fail(
      "Director Phase P2 Wave 2 dual execution chain gate failed.",
